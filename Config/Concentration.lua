-- This file is almost entirely lifted from CraftSim ConcentrationData.lua
...
---@class CraftScan.ConcentrationData
---@overload fun(currencyID : number): CraftScan.ConcentrationData
CraftScan.ConcentrationData = CraftScan.Object:extend()

---@param currencyID number
function CraftScan.ConcentrationData:new(currencyID)
    self.currencyID = currencyID

    local currencyInfo = C_CurrencyInfo.GetCurrencyInfo(self.currencyID)
    self.lastUpdated = GetServerTime() -- is in seconds with ms precision
    self.amount = currencyInfo and currencyInfo.quantity or 0
    self.maxQuantity = currencyInfo and currencyInfo.maxQuantity or 0
    self.rechargingDurationMS = currencyInfo and currencyInfo.rechargingCycleDurationMS or 0
end

---@return CraftScan.ConcentrationData
function CraftScan.ConcentrationData:Copy()
    local copy = CraftScan.ConcentrationData(self.currencyID)
...
function CraftScan.ConcentrationData:GetCurrentAmount()
...
function CraftScan.ConcentrationData:GetTimeUntil(concentrationValue)
...
---@return number time in seconds with ms precision
function CraftScan.ConcentrationData:GetTimeUntilMax()
...
function CraftScan.ConcentrationData:GetFormattedTimerUntil(concentrationValue)
...
---@return string formattedTimer
---@return boolean ready
function CraftScan.ConcentrationData:GetFormattedTimerUntilMax()
    return self:GetFormattedTimerUntil(self.maxQuantity)
end

function CraftScan.ConcentrationData:GetTimestampMax()
    return GetServerTime() + self:GetTimeUntilMax()
end

function CraftScan.ConcentrationData:GetFormattedDateMax()
...
function CraftScan.ConcentrationData:OnCooldown()
    return self:GetCurrentAmount() < self.maxQuantity
end

---@class CraftScan.ConcentrationData.Serialized
...
function CraftScan.ConcentrationData:Serialize()
    ---@type CraftScan.ConcentrationData.Serialized
    local serialized = {
...
---@param serialized CraftScan.ConcentrationData.Serialized
---@return CraftScan.ConcentrationData
function CraftScan.ConcentrationData:Deserialize(serialized)
    local concentrationData = CraftScan.ConcentrationData(serialized.currencyID)
