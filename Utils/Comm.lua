...
local function ReceiveShareCustomGreeting_(greeting)
    if
        not CraftScan.DB.settings.greeting
        or ((CraftScan.DB.settings.greeting.rev or 0) < greeting.rev)
    then
...
local function ReceiveShareCustomExplanations_(explanations)
    if
        not CraftScan.DB.settings.explanations
        or ((CraftScan.DB.settings.explanations.rev or 0) < explanations.rev)
    then
...
CraftScan.Events:Register('CHARACTER_ENABLED', function(ctxt)
    CraftScanComm:ShareCharacterModification(ctxt.name, ctxt.ppInfo.professionID)
end)

function CraftScanComm:ShareCustomerOrder(message, customer, customerGuid, lastChatFrameMessage)
    if not HaveTarget() or not LinkedAccountsConfigured() then
        return
    end
    if CraftScanComm.applying_remote_state then
        return
    end -- Block infinite recursion
    if not CraftScan.DB.settings.proxy_send_enabled then
        return
    end -- We're disabled
...
local function ReceiveShareCustomerOrder(sender, data, senderID)
    if not CraftScan.DB.settings.proxy_receive_enabled then
        return
    end
    if
        not CraftScan.InjectLastChatFrameMessage(
            data.customer,
            data.message,
            data.lastChatFrameMessage
        )
...
function CraftScanComm:ShareCustomExplanations(explanations)
    ...
    CraftScanComm:Transmit(
        explanations,
        CraftScanComm.Operations.ShareCustomExplanations,
        TARGET_ALL
    )
end
...
local function ShareAnalytics_(senderID, sender, state, sinceLastUpdate)
    ...
    CraftScan.Utils.printTable(
        'ShareAnalytics_ Account',
        CraftScan.DB.realm.linked_accounts[senderID]
    )
    ...
    CraftScanComm:Transmit(
        ...
        CraftScanComm.Operations.ShareAnalytics,
        sender
    )
end
            ...
            if otherItem.ppID then
                ...
                elseif otherItem.ppID ~= myItem.ppID then
                    ...
                    item:ContinueOnItemLoad(function()
                        ...
                        local otherProf =
                            CraftScan.Utils.ColorizedProfessionNameByID(otherItem.ppID)
                        print(
                            string.format(
...
function CraftScanComm:AcceptPeerRequest(nickname)
    ...
    }, CraftScanComm.Operations.Handshake, CraftScan.pending_peer_request.character)
...
function CraftScanComm:RequestCraft(target, itemID)
    ...
    CreateRequestCraftGreeting(itemID, function(message)
        ...
        CraftScanComm:Transmit(
            ...
            CraftScanComm.Operations.RequestCraft,
            target
        )
    end)
end
...
local function IsPublicOperation(op)
    return CraftScanComm.Operations.FindCrafter == op or CraftScanComm.Operations.RequestCraft == op
end
...
local function ReceiveRemoteTarget(senderID, sender)
    ...
    if
        not CraftScan.DB.characters[nameAndRealm]
        and not CraftScan.Utils.Contains(backupChars, sender)
    then
    ...
    if msg.operation == CraftScanComm.Operations.Handshake then
        ...
        if msg.version ~= CraftScan.CONST.CURRENT_VERSION then
            if
                ...
                or CraftScan.IsRemoteVersionMoreRecent(msg.version)
            then
                print(
                    string.format(
        ...
        local hasFull =
            CraftScan.Utils.Contains(linkedAccount.permissions, CraftScanComm.Permissions.Full)
        local hasAnalytics =
            CraftScan.Utils.Contains(linkedAccount.permissions, CraftScanComm.Permissions.Analytics)
        ...
        elseif
            ...
            and msg.operation == CraftScanComm.Operations.ShareAnalytics
        then
...
function CraftScanComm:OnCommReceived(prefix, payload, distribution, sender)
    ...
    if prefix ~= CRAFT_SCAN_COMM_PREFIX then
        return
    end
