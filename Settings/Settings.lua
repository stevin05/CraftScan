...
function CraftScanSettingsMultiSelectDropDownMixin:SetupDropdownMenu(
    button,
    setting,
    options,
    initTooltip
...
local function CreateMultiSelectDropdown(category, variable, key, name, options, tooltip)
    ...
    local initializer = Settings.CreateControlInitializer(
        'CraftScanSettingsMultiSelectDropDownTemplate',
        setting,
        options,
        tooltip
    )
...
CraftScan.Utils.onLoad(function()
    ...
    do
        ...
        CreateMultiSelectDropdown(
            category,
            'CRAFT_SCAN_ADDON_WHITELIST',
            'disabled_addon_whitelist',
            L(LID.ADDON_WHITELIST_LABEL),
            GetOptions,
    ...
    do
        ...
        CreateDropdown(
            category,
            'CRAFT_SCAN_PING_SOUND',
            'ping_sound',
            L(LID.PING_SOUND_LABEL),
            GetOptions,
    ...
    do
        ...
        CreateDropdown(
            category,
            'CRAFT_SCAN_BANNER_DIRECTION',
            'banner_direction',
            L(LID.BANNER_SIDE_LABEL),
            GetOptions,
    ...
    do
        ...
        options:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right, function(value)
            return value .. ' ' .. (value == 1 and L('Minute') or L('Minutes'))
        end)
        CreateSlider(
            category,
    ...
    do
        ...
        options:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right, function(value)
            return value .. ' ' .. (value == 1 and L('Second') or L('Seconds'))
        end)
        CreateSlider(
            category,
    ...
    do
        ...
        local initializer =
            Settings.CreateCheckbox(category, setting, L(LID.PERMISSIVE_MATCH_SETTING))
        initializer:AddSearchTags(L(LID.CRAFT_SCAN))
    end
    do
        local GetValue = function()
